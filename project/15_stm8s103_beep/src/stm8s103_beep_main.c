...
/*
  info :
  이 기능은 LS 클럭이 128 kHz 주파수로 동작할 때 1, 2 또는 4 kHz 범위의 삐 소리를 생성합니다.
  삐 소리 기능을 사용하려면 다음 단계를 순서대로 수행하세요: 
  1. 13.2.2절에 설명된 LS 클럭 주파수를 보정: BEEPDIV[4:0] 값을 정의하기 위한 비퍼 보정.
  2. 비퍼 제어/상태 레지스터(BEEP_CSR)의 BEEPSEL[1:0] 비트에 쓰어 1 kHz, 2 kHz 또는 4 kHz 출력 주파수를 선택합니다. 
  3. 비퍼 제어/상태 레지스터(BEEP_CSR)에서 LS 클럭 소스를 활성화하기 위해 BEEPEN 비트를 설정합니다.
*/
            ...
            if (index > 0)
            {
              ...
              if (found >= 2)
              {
                ...
                if (strcmp(cmd, "beep") == 0)
                {
                  ...
                  fnBeepStart((uint16_t)value_a,(uint16_t)value_b);
                }
                else
                {
                  mib_printf("\r\n Unknown command: %s\r\n", cmd);
...
{
  MibWriteDebugStringCheck(1, " #######################################\r\n");
  MibWriteDebugStringCheck(1, " # PD4 : 1KHz 1sec delay               #\r\n");
  MibWriteDebugStringCheck(1, " # beep [frequency]Hz [delay stop]msec #\r\n");
  MibWriteDebugStringCheck(1, " # beep 2000 2000      2 KHz 2 Sec     #\r\n");
  MibWriteDebugStringCheck(1, " # beep 4000 3000      4 KHz 3 Sec     #\r\n");
  MibWriteDebugStringCheck(1, " #######################################\r\n");
}
//----------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------
