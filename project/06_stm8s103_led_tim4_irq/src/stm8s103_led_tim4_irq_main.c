/**
 ******************************************************************************
 * @file stm8s103_led_tim4_irq_main.c
...
/*
    info :
TIM4_Overflow_cb (타이머1에서 오버플로우 발생 시 호출)
타이머 4의 1msec마다 인터럽트를 발생시켜서 1 sec를 체크하여 깜빡이도록 하는 기능을 구현한 예제이다.
채터링 방지기능 함수를 포함하고 있다. 키가 눌리면 현 LED 상황을 반전 시키고 키가 떼어질 때 부터 1초 간격으로 LED가 깜빡인다.
