  ...
  }
  {
    ...
    do
    {
      ...
      }
      {
        ...
        {
          gbLedFlag = 1; // 키 입력이 있을 때 LED 토글 방지 위해 플래그 설정
          // 2. \n(또는 \r)을 만났을 때 처리
          if (vwCh == '\r' || vwCh == '\n')
          {
...
void GPIO_Configuration(void)
{
  /* Configure PD1 (LED1) as output push-pull low (led switched on) */
